package app.braintropy.extension.myoadapt;
 ...
 *
 * Two halves:
 *  - spoofCustomerInfoJson: rewrites the RevenueCat CustomerInfo JSON before
 *    the SDK parses it (called from CustomerInfoFactory.buildCustomerInfo).
 *  - install: registers an OkHttp client factory on React Native's
 *    OkHttpClientProvider that adds an application interceptor forcing the
 *    coach.myoadapt.app backend gate payloads (GetUserInfo, login,
 *    subscription endpoints) to report an active subscription.
 *
 * Based on the MyoAdapt patch by Xhehab (GPL-3.0):
 * https://github.com/Xhehab/Xhehab-Patches
 * Modified to never touch app-local storage. The original wiped
 * files/mmkv on every cold start, racing MMKV init and randomly
 * logging the user out.
 */
public final class MyoAdaptUnlock {
    private static final String TAG = "MyoAdaptUnlock";
    ...
    private static void installOkHttpFactory() throws Exception {
                        ...
                        new InvocationHandler() {
                            ...
                            public Object invoke(Object proxy, Method method, Object[] args)
                                ...
                                if ("createNewNetworkModuleClient".equals(method.getName())) {
                                    ...
                                    // Application interceptor (NOT network): the body is
                                    // already gunzipped at this layer.
        ...
        // Drop any client created before our factory was installed.
